...
def tier_col(t): return {"CHAMPION":"#059669","INFLUENCER":"#3B82F6","GATEKEEPER":"#F97316","UNKNOWN":"#6B7280"}.get(t,"#6B7280")
...
if analyse_btn:
    ...
    m2.metric("Person Score",   f"{pe_score}/100", pe_tier)
    ...
    with colB:
        ...
        <div class="intel-card">
          ...
          <span class="tag" style="background:{tc}22;color:{tc};border:1px solid {tc}44">{pe_tier}</span>
    ...
    # ── SALES ACTION ───────────────────────────────────────────────────────────
    st.markdown(f"""<div class="action-card"><div class="card-header" style="color:#6B7280;font-size:.7rem;font-weight:700;text-transform:uppercase;letter-spacing:1.2px;border-bottom:1px solid #1F2937;padding-bottom:7px;margin-bottom:14px">💡 Sales Action Plan</div></div>""", unsafe_allow_html=True)

    a1,a2,a3 = st.columns(3)
    with a1:
        st.markdown("**📧 Suggested Subject Line**")
        subj = act.get("suggested_subject_line","—")
        st.markdown(f'<div class="code-box">{subj}</div>',unsafe_allow_html=True)
        st.code(subj, language=None)
        st.markdown("**🎯 Opening Angle**")
        st.markdown(f"<div class='detail'>{act.get('opening_angle','—')}</div>",unsafe_allow_html=True)
    with a2:
        st.markdown("**🔍 Specific Hook**")
        st.markdown(f"<div class='hbox' style='border-left-color:#8B5CF6'>{act.get('specific_hook','—')}</div>",unsafe_allow_html=True)
        st.markdown("**▶ Next Step**")
        ns = act.get("suggested_next_step","—").replace("_"," ")
        st.markdown(f"<div class='detail'><span style='color:#3B82F6'>▶</span> <strong>{ns}</strong></div>",unsafe_allow_html=True)
        if act.get("next_step_detail"):
            st.markdown(f"<div class='detail'><span>  </span>{act['next_step_detail']}</div>",unsafe_allow_html=True)
    with a3:
        st.markdown("**⚠️ Caution Flags**")
        cautions = act.get("caution_flags",[])
        if cautions:
            for c_ in cautions: st.markdown(f'<div class="caution">⚡ {c_}</div>',unsafe_allow_html=True)
        else:
            st.markdown('<div class="caution" style="background:#064E3B;border-color:#065F46;color:#34D399">✓ No major caution flags</div>',unsafe_allow_html=True)
        st.markdown("**💬 Contact Strategy**")
        st.markdown(f"<div class='detail'>{act.get('ideal_contact_strategy','—')}</div>",unsafe_allow_html=True)

    if act.get("priority_reasoning"):
        st.markdown(f"<div class='hbox' style='border-left-color:{hx};margin-top:.8rem'><strong>Why {priority.replace('_',' ')}?</strong><br>{act['priority_reasoning']}</div>",unsafe_allow_html=True)
